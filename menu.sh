...
# USERNAME
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/Scvpsss/izin/main/vps | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
# Order ID
rm -f /usr/bin/ver
user=$(curl https://raw.githubusercontent.com/Scvpsss/izin/main/vps | grep $MYIP | awk '{print $3}')
echo "$user" >/usr/bin/ver
# validity
rm -f /usr/bin/e
valid=$(curl https://raw.githubusercontent.com/Scvpsss/izin/main/vps | grep $MYIP | awk '{print $4}')
...
Exp1=$(curl https://raw.githubusercontent.com/Scvpsss/izin/main/vps | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
    sts="${Info}"
else
    sts="${Error}"
